...
class MCPToolKit:
    ...
    def from_endpoints(
        ...
        return cls(
            ...
            _all=[npi, icd10, cms, fhir, pubmed, clinical_trials],
            _http_client=client,
        )
    ...
    def coverage_tools(self) -> list[MCPStreamableHTTPTool]:
        """Tools for the Coverage Agent: CMS + ICD-10 search."""
        return [
    ...
    def all_tools(self) -> list[MCPStreamableHTTPTool]:
        """All 6 MCP tools — for the top-level Healthcare Orchestrator."""
        return [
    ...
    def prior_auth_tools(self) -> list[MCPStreamableHTTPTool]:
        """All tools for the Prior Auth Orchestrator (all 6 MCP servers)."""
        return [
