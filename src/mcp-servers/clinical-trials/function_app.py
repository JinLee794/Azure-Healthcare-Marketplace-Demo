...
TOOLS = [
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "Search query (condition, drug, device, or keywords)"},
                "condition": {
                    "type": "string",
                ...
                "phase": {
                    "type": "array",
                    "items": {"type": "string", "enum": ["EARLY_PHASE1", "PHASE1", "PHASE2", "PHASE3", "PHASE4", "NA"]},
                    "description": "Filter by trial phase",
                },
                "location_country": {"type": "string", "description": "Filter by country (e.g., 'United States')"},
                "location_state": {"type": "string", "description": "Filter by US state (e.g., 'California')"},
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {"nct_id": {"type": "string", "description": "NCT identifier (e.g., 'NCT04280705')"}},
            "required": ["nct_id"],
        },
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {"nct_id": {"type": "string", "description": "NCT identifier"}},
            "required": ["nct_id"],
        },
    },
    {
...
async def search_trials(
    ...
    params = {"format": "json", "pageSize": min(page_size, 100)}
    ...
    # Request fields
    params["fields"] = (
        "NCTId,BriefTitle,OverallStatus,Phase,Condition,InterventionName,LocationCity,LocationState,LocationCountry,StartDate,CompletionDate,EnrollmentCount,StudyType,LeadSponsorName"
    )
...
async def search_by_condition(condition: str, location: Optional[str] = None, distance_miles: int = 50) -> dict:
...
async def get_trial_results(nct_id: str) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        return {
            ...
            "results_posted_date": protocol.get("statusModule", {}).get("resultsFirstPostDateStruct", {}).get("date")
            if has_results
            else None,
...
def _format_trial_detail(data: dict) -> dict:
    ...
    return {
        ...
        "central_contacts": [
            {"name": c.get("name"), "role": c.get("role"), "phone": c.get("phone"), "email": c.get("email")}
...
def _extract_outcomes(protocol: dict, results: dict) -> list:
    ...
    return [
        {"title": o.get("title"), "description": o.get("description"), "time_frame": o.get("timeFrame")}
        for o in primary[:5]
    ]
    ...
    try:
        if method == "initialize":
            ...
            if tool_name == "search_trials":
                ...
                tool_result = await search_by_condition(
                    args.get("condition", ""), args.get("location"), args.get("distance_miles", 50)
                )
